...
### 9.x 在 OpenClaw 镜像中默认安装 `oci-cli`（供运行时 bash/tool 调用）

如果你的 OpenClaw agent 在运行时会通过 bash/tool 执行 `oci ...`（而不是人工 `kubectl exec` 调试），那么 **运行 OpenClaw 进程的容器镜像里必须存在 `oci` 可执行文件**。

- 仅增加一个“debug sidecar 容器”通常**不够**：因为 bash/tool 默认在 OpenClaw 运行容器内执行，不能自动跨容器调用另一个容器里的 `/usr/local/bin/oci`。

> 说明：你提供的 Oracle 文档页面未包含 Debian/Ubuntu 的 apt 仓库安装流程；对 Ubuntu 的推荐是使用 GitHub 上的 `install.sh` 安装脚本。

#### 推荐 Dockerfile 片段（runtime stage，默认安装，使用 `install.sh`）

在 OpenClaw 源码仓库的 `../openclaw/Dockerfile` 中，把以下片段放在 runtime stage：
- `FROM base-${OPENCLAW_VARIANT}` 之后
- `USER node` 之前

```dockerfile
# --- Install OCI CLI (default) ---
# Pin the installer to a specific ref for reproducible builds.
# You can set e.g. OPENCLAW_OCI_CLI_INSTALL_REF=v3.x.y or a commit SHA.
ARG OPENCLAW_OCI_CLI_INSTALL_REF="master"
ARG OPENCLAW_OCI_CLI_INSTALL_DIR="/opt/oci-cli"

RUN --mount=type=cache,id=openclaw-oci-cli-cache,target=/root/.cache,sharing=locked \
    set -eux; \
    apt-get update; \
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ca-certificates curl python3 python3-venv; \
    rm -rf /var/lib/apt/lists/*; \
    curl -fsSL "https://raw.githubusercontent.com/oracle/oci-cli/${OPENCLAW_OCI_CLI_INSTALL_REF}/scripts/install/install.sh" -o /tmp/oci_install.sh; \
    bash /tmp/oci_install.sh -- --accept-all-defaults --install-dir "${OPENCLAW_OCI_CLI_INSTALL_DIR}"; \
    ln -sf "${OPENCLAW_OCI_CLI_INSTALL_DIR}/bin/oci" /usr/local/bin/oci; \
    rm -f /tmp/oci_install.sh
```

#### 注意事项
- 为了构建可复现/供应链可控，建议把 `OPENCLAW_OCI_CLI_INSTALL_REF` 从 `master` 固定为 **release tag** 或 **commit SHA**。
- 安装 `oci-cli` 会增加镜像体积（Python + 依赖）。
- 在 OKE 内要让 `oci` 真正调用成功，还需要配套：
  - Workload Identity（`OCI_RESOURCE_PRINCIPAL_VERSION` / `OCI_RESOURCE_PRINCIPAL_REGION` 等）
  - IAM policy 授权到 OpenClaw 的 ServiceAccount
  - NetworkPolicy 放开到 OCI endpoints 的 443 egress（若启用了 egress 限制）
